'''
Modified from https://github.com/carolinepacheco/convert-yolo-to-pascalvoc
'''
import os
import cv2
...
## -------------------- defined Image Extension --------------------
img_ext = '.png'

## -------------------- defined Dataset Name --------------------
dataset_name = 'Test_Recycle_Waste'

## -------------------- defined path 
fullpath = './RecycleWasteDatasetYolo/data' ## << don't end with /

## -------------------- defined folder
txt_img_folder = 'train'

## -------------------- defined output (xml) folder
...
def xml_transform(root, classes):  
    ...
    imgpath = join(root, '%s'+img_ext)
    
    os.makedirs(join(root,xml_folder), exist_ok=True)
    ...
    for i in range(len(ids)):
        ...
        node_folder.text = txt_img_folder
        ...
        node_database.text = dataset_name
